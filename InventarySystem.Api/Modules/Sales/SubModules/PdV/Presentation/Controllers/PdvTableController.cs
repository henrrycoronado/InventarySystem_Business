...
[ApiController]
[Route("api/companies/{companyId:int}/pdv/tables")]
public class PdvTableController(IPdvTableService service) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetAll(int companyId)
    {
        var result = await service.GetAllByCompanyAsync(companyId);
        return Ok(ApiResponse<IEnumerable<PdvTableDto>>.Ok(result));
    }
    ...
    public async Task<IActionResult> GetById(int companyId, int id)
    {
        ...
        if (result is null) return NotFound(ApiResponse<object>.Fail($"Table {id} not found"));
        return Ok(ApiResponse<PdvTableDto>.Ok(result));
    }

    [HttpPost]
    public async Task<IActionResult> Create(int companyId, [FromBody] PdvTableCreateDto dto)
    {
        dto.CompanyId = companyId;
        var result = await service.CreateAsync(dto);
        return CreatedAtAction(nameof(GetById), new { companyId, id = result.Id }, ApiResponse<PdvTableDto>.Ok(result));
    }
    ...
    public async Task<IActionResult> Deactivate(int companyId, int id)
    {
        ...
        {
            return NotFound(ApiResponse<object>.Fail($"Table {id} not found"));
        }
    }
}